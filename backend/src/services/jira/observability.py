...
_TRACER = trace.get_tracer("src.services.jira") if trace else None
_METER = metrics.get_meter("src.services.jira") if metrics else None
...
def _render_json(payload: Any) -> str:
    if structlog is not None:
        ...
        rendered = renderer({}, "event", payload)
        if isinstance(rendered, (bytes, bytearray, memoryview)):
            return bytes(rendered).decode("utf-8")
        return str(rendered)
