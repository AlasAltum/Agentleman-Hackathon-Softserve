...
def create_ticket(
    ...
    *,
    ...
    When llm_title / llm_description are provided (generated by the triage LLM),
    they are used as the Jira summary and description. Otherwise the raw incident
    report fields are used as a fallback.
    """
    ...
    summary = llm_title or _build_issue_summary(preprocessed)
    description = (
        _build_adf_from_markdown(llm_description, reporter_email=reporter_email, request_id=active_request_id)
        if llm_description
        else _build_issue_document(preprocessed)
    )
    ...
    with traced_operation(
        ...
        log_event(
            ...
            severity=triage.severity.value,
            incident_type=triage.classification.incident_type.value,
            summary_source="llm" if llm_title else "raw",
        )
...
def _build_adf_from_markdown(
    text: str,
    *,
    reporter_email: str | None = None,
    request_id: str | None = None,
) -> dict[str, Any]:
    """Convert the LLM-generated description into Atlassian Document Format.

    Prepends reporter metadata (email, request_id) as the first block so
    responders always know who filed the ticket. Then parses:
      - h2. Heading         → ADF heading level 2
      - * bullet / - bullet → ADF bulletList
      - blank line          → flush pending bullet list
      - anything else       → ADF paragraph
    """
    content: list[dict[str, Any]] = []
    current_bullets: list[str] = []

    # Always include reporter metadata at the top
    meta_lines = []
    if reporter_email:
        meta_lines.append(f"Reporter: {reporter_email}")
    if request_id:
        meta_lines.append(f"Request ID: {request_id}")
    if meta_lines:
        content.append(
            {
                "type": "paragraph",
