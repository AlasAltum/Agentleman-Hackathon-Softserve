...
from src.workflow.models import IncidentType, TicketInfo, TriageResult


def _create_or_update_ticket(triage: TriageResult, reporter_email: str) -> TicketInfo:
    ...
    if triage.classification.incident_type == IncidentType.ALERT_STORM:
        return _update_existing_ticket(triage, reporter_email)
    return _create_new_ticket(triage, reporter_email)


def _create_new_ticket(triage: TriageResult, reporter_email: str) -> TicketInfo:
    ...
    # TODO: call Jira API — create issue with technical_summary and severity
    return TicketInfo(
        ticket_id=ticket_id,
...
def _update_existing_ticket(triage: TriageResult, reporter_email: str) -> TicketInfo:
